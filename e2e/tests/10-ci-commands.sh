...
PAGES_MD=$(git-prompt-story ci-summary "${INITIAL_COMMIT}..HEAD" --format=markdown --pages-url="https://example.github.io/repo/pr-42/")
...
rm -rf /tmp/html-test
git-prompt-story ci-html "${INITIAL_COMMIT}..HEAD" --output-dir=/tmp/html-test --pr=42

test -f /tmp/html-test/index.html || fail "index.html should be created"
echo "    - index.html created"

# Check that commit pages were created (short SHA format)
SHORT_SHA1=$(echo "$COMMIT1" | cut -c1-7)
SHORT_SHA2=$(echo "$COMMIT2" | cut -c1-7)

test -f "/tmp/html-test/${SHORT_SHA1}.html" || fail "${SHORT_SHA1}.html should be created"
echo "    - ${SHORT_SHA1}.html created"

test -f "/tmp/html-test/${SHORT_SHA2}.html" || fail "${SHORT_SHA2}.html should be created"
echo "    - ${SHORT_SHA2}.html created"
...
rm -f /tmp/summary.md
git-prompt-story ci-summary "${INITIAL_COMMIT}..HEAD" --format=markdown --output=/tmp/summary.md
...
NO_NOTES_OUTPUT=$(git-prompt-story ci-summary "${COMMIT2}..HEAD" --format=json)
